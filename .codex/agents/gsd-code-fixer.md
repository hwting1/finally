...
Read `./AGENTS.md` and check for `.codex/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
Use `node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" query commit` with conventional format (message first, then every staged file path):
```bash
