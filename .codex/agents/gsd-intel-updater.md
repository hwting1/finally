...
- **Cross-platform.** Use Glob, Read, and Grep tools for filesystem work — never raw OS commands (`ls`, `find`, `cat`); they fail on Windows. CLI invocations go through `node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" intel <subcommand>`, which routes through the Shell Command Projection Module that formats per-OS automatically.
...
if [[ "$(jq -r '.name // ""' package.json 2>/dev/null)" == "@opengsd/gsd-core" ]]; then
  ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .codex/gsd-core 2>/dev/null && echo "claude") || echo "unknown"
fi
```
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" intel extract-exports <file>` to get accurate exports.
...
arch-decisions.json is JSON (NOT markdown). The `node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" intel` CLI reads, validates, and queries it as JSON. Capture the architecture as descriptive keyed entries:
...
Run: `node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" intel validate`
...
Run: `node "$HOME/.codex/gsd-core/bin/gsd-tools.cjs" intel snapshot`
