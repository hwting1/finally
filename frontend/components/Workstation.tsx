...
import type { ChatActionResult, ChatMessage, PortfolioResponse, PortfolioSnapshot, Position, PriceQuote, PriceStreamPayload, TradeSide } from "@/lib/types";
...
function formatChatAction(action: ChatActionResult): string {
  const ticker = action.requested?.ticker ? ` ${action.requested.ticker}` : "";
  const side = action.requested?.side ? ` ${action.requested.side}` : "";
  const quantity = action.requested?.quantity ? ` ${formatNumber(action.requested.quantity, 4)}` : "";
  const actionLabel = `${action.status} ${action.type}${side}${quantity}${ticker}`.trim();
  return `${actionLabel}: ${action.message}`;
}
    ...
    try {
      ...
      setMessages((current) => [
        ...current,
        {
          ...
          actions: response.actions?.map(formatChatAction)
        }
      ]);
      getPortfolio()
        .then((payload) => {
      ...
      <div className="grid">
        ...
        <section className="panel trade" data-testid="trade-bar">
          ...
          <div className="trade-controls">
            ...
            <button data-testid="buy-button" className="buy" type="button" onClick={() => void submitTrade("buy")}>Buy</button>
            <button data-testid="sell-button" className="sell" type="button" onClick={() => void submitTrade("sell")}>Sell</button>
          </div>
        </section>
